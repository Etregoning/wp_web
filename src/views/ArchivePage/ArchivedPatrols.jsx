...
import { requestData, requestDelete } from "api/ReportsAPI";
...
class ArchivedPatrols extends React.Component {
  constructor() {
    super();
    this.state = {
      data: [],
    ...
    this.fetchIssues = this.fetchPatrols.bind(this);
    this.deleteIssues = this.deletePatrols.bind(this);
            ...
            },
            {
              ...
              Cell: (rowInfo ) => 
                ...
                rowInfo.row.status === "urgent" ? (
                  <IconButton style={{height:30, width: 30, margin: 0, padding: 0}}
                    onClick={() =>  this.setUrgency( rowInfo.row.id, rowInfo.row.status)}
                  >
                    <TiWarning  color='red'/>
                  </IconButton>
              ...
              filterMethod: (filter, row) => {
                if (filter.value === "all") return row[filter.id] === null || row[filter.id] === "urgent";
               if (filter.value === "urgent") return row[filter.id] === "urgent";
              },
...
export default withStyles(dashboardStyle)(ArchivedPatrols);